...
public class FileUploaderModel implements FileUploaderContract.Model {
    ...
    public Single<ResponseBody> uploadImageWithoutProgress(ProductReqeust request) {
        return service.postImage(parseRequestBody(request.getDesc(), "desc"), parseRequestBody(String.valueOf(request.getPrice()), "price"), parseRequestBody(request.getGroupName(), "group"), createMultipartBody(request.getImg()));
    }
...
//        parts.add(createMultipartBody(request.getImg()));
        return Flowable.create(emitter -> service.postImage(parseRequestBody(request.getDesc(), "desc"), parseRequestBody(String.valueOf(request.getPrice()), "price"), parseRequestBody(request.getGroupName(), "group"), createMultipartBody(request.getImg(), emitter)).subscribe(result -> emitter.onComplete(), emitter::onError), BackpressureStrategy.LATEST);
    }
